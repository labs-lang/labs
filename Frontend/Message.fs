...
type Message<'a> = { What: 'a; Where: Position list }

let private _pprintWithPos header msg (where: Position list) =
    let lineColumn (p: Position) =
        if p.Line > 0 then $":{p.Line}:{p.Column}" else ""

    let pos =
        where
        |> List.map (fun p -> $"{p.StreamName}{lineColumn p}")
        |> String.concat "; "
...
let map f (d: Node<_>) =
    try
        { Pos = d.Pos
          ...
          Def = f d.Def
          Source = d.Source }
    with :? LabsException as e ->
        raise (LabsException { e.Data0 with Where = [ d.Pos ] })
