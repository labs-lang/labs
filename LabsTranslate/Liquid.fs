...
let private internalRender strfun (template: Template) values =
    ...
    and hashdict x =
        Seq.map (fun (k, v) -> k, hashval v) x |> dict |> Hash.FromDictionary
    ...
    if template.Errors.Count = 0 then
        ...
        template.Errors
        |> Seq.map (fun x ->
            { What = Codegen x.Message
              Where = [ Position(x.Source, -1, -1, -1) ] })
...
let parse path =
    ...
    try
        File.ReadAllText path |> Template.Parse
    with ex ->
        failwith $"Parsing failed: {path}: {ex.Message}"
