...
let private refTypeCheck v (offset: 'a option) =
    ...
    let test, msg =
        ...
        | C1Ref
        | C2Ref -> offset.IsSome, $"Constant {v.Vartype} treated as Array"
...
let getLstigVars expr =
    getVars expr |> Set.filter (fun (v, _) -> isLstigVar v)
...
let rec collectAux trExpr expr =
    ...
    | Nondet(e1, e2, pos) ->
        recurse e1
        |> Set.union (recurse e2)
        |> Set.add ($"nondet_{pos.Line}_{pos.Column}", trExpr e1, trExpr e2)
    | IfElse(_, e1, e2) // TODO collect auxs in condition too
    | Arithm(e1, _, e2) -> recurse e1 |> Set.union (recurse e2)
    | Unary(_, e) -> recurse e
    | Ref r ->
        r.Offset
        |> Option.map (Set.unionMany << List.map recurse)
...
let translateBExpr bleafFn negFn compareFn compoundFn bexpr =
    bexpr
    |> cata bleafFn negFn compareFn compoundFn (fun _ _ _ -> failwith $"Unexpected {tFOREACH}")
        ...
        let propRef1 ((v: Var<_>, i), c) ref_ofAgent offset ofagent =
            ...
            | None, None ->
                ...
                if v.Location <> E then
                    failwithf $"{v.Name} is not an environment variable ({v.Location})"
        ...
        if not prop.Quantifiers.IsEmpty then
            ...
            let translateWithSubs s =
                trProp
                    s
                    { prop with
                        Quantifiers = prop.Quantifiers.Remove nextId }
...
type ITranslateConfig =
    ...
    abstract member TrExpr<'a, 'b> :
        RefTranslator<'a> -> ('b -> string) -> (BExpr<'a, 'b> -> string) -> Expr<'a, 'b> -> string

    abstract member TrBExpr<'a, 'b when 'a: comparison and 'b: comparison> :
        (Expr<'a, 'b> -> string) -> BExpr<'a, 'b> -> string
...
let makeTranslationKit (conf: ITranslateConfig) =
    ...
    let rec mainGuardTr bexpr =
        let tr =
            conf.TrExpr (trref conf.TrLoc conf.TrLinkId "firstAgent") (fun () -> conf.AgentName) mainGuardTr
    ...
    let rec agentExprTr expr =
        conf.TrExpr
            (trref conf.TrLoc conf.TrLinkId conf.AgentName)
            (fun () -> conf.AgentName)
            (guardTr agentExprTr)
            expr
    ...
    let propTr = translateProp conf.TrExpr conf.TrBExpr conf.TrLoc conf.TrLinkId
    let qpredTr = translateQPred conf.TrExpr conf.TrBExpr conf.TrLoc conf.TrLinkId ""
...
module internal C =
    ...
    let rec private trBExprC trExpr andOp orOp bexpr =
        ...
        let compareFn e1 op e2 =
            $"((%s{trExpr e1}) {op} (%s{trExpr e2}))" //TODO
...
module internal Lnt =
    ...
    let wrapperParallel =
        { new ITranslateConfig with
            member _.TemplateInfo =
                { BaseDir = "templates/lnt-parallel"
                  Extension = "lnt" }
