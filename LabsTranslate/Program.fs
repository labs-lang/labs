...
let wrapParserResult p text =
    try
        ...
        | Failure(errorMsg, _, _) -> Outcome.Error([], [ { What = Parser errorMsg; Where = [] } ])
    with ex ->
        Outcome.Error(
            [],
            [ { What = Generic ex.Message
                Where = [] } ]
        )
...
let main argv =
    try
        ...
        <~> fun cli ->
            ...
            <~> Frontend.run externs
            <~> fun x -> zero (cli, x)
            <?> fun (cli, table) ->
                if cli.Contains Info then
                    zero (table.Dump prop |> printf "%s")
                elif cli.Contains Only_Ast then
                    { table with
                        Info = Some <| table.Dump prop }
                    |> fun j -> JsonSerializer.Serialize(j, JsonOptions table)
                    |> printfn "%s"
                    |> zero
                else
