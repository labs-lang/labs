...
let simpleRef p =
    tuple3
        ...
        (choice [ followedBy OF >>. ws OF >>. p |>> Some; notFollowedBy OF >>% None ])
    |>> fun (str, offset, ofAgent) ->
        { Var = str
          Offset = offset
          OfAgent = ofAgent }
...
let makeBExprParser pexpr =
    ...
    opp.AddOperator(
        InfixOperator("=", ws <| notFollowedBy (skipChar '>'), 2, Associativity.Left, ParseBExpr.compare Equal)
    )
...
let makeExprParser pref pid pbexpr : Parser<_> =
    ...
    let prawcall =
        ...
        RAWPREFIX >>. identifier opts .>>. betweenParen (sepBy expr (ws COMMA))
    ...
    let pcond =
        ...
        >>. tuple3 (ws (safeSkip tIF) >>. pbexpr) (ws (safeSkip tTHEN) >>. expr) (ws (safeSkip tELSE) >>. expr)
        |>> IfElse
        <!> "if-then-else"

    opp.TermParser <-
        [ term; pmax; pmin; pcond; (betweenParen expr) <!> "paren" ]
        |> List.map ws
        |> choice
    ...
    opp.AddOperator(
        InfixOperator(tROUNDDIV, notFollowedBy (skipChar '=') |> ws, 2, Associativity.Left, arithm RoundDiv)
    )
