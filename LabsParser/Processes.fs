...
let rec private _pe () =
    makeExprParser simpleRef (skipString tID .>> notInIdentifier) _pb
...
let paction =
    ...
    let pCount =
        ...
        >>. pipe3 (countToken >>. ws IDENTIFIER) (ws KEYNAME .>> (ws COMMA)) pguard (fun typ name bexpr ->
            Location.Local, [ Count(typ, name, bexpr) ])
    ...
    tuple2 (ws pSingleLhs |> sepbycommas) (ws parseArrow <|> pWalrus)
    >>= fun (refs, (loc, exprs)) ->
        try
            { ActionType = loc
              Updates = List.zip refs exprs }
            |> preturn
    ...
    pprocRef.Value <-
        let doBase (pos, stmt) =
            { Def = stmt
              Pos = pos
              Source = ""
              Name = string stmt |> _.Trim() }
            |> BaseProcess

        let compose a b = Comp(a, b)

        let pGUARDORFATGUARD =
            choice
        ...
        let pGuarded =
            ...
            >>. pipe4 getPosition (ws pguard) (ws pGUARDORFATGUARD) pproc (fun pos g typ p ->
                typ
                    { Pos = pos
                      Name = ""
                      Source = ""
        ...
        let pseq = sepBy1 (ws pBase) (ws SEQ) |>> compose Seq <!> "SEQ"
        let pchoice = sepBy (ws pseq) (ws CHOICE) |>> compose Choice <!> "CHOICE"
