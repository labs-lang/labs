...
let ws p =
    p .>> spaces .>> skipMany (spaces1 <|> lineComment)
...
let safeIdentifier options =
    ...
    identifier options
    >>= fun x ->
        if List.contains x reserved then
            fail $"Unexpected keyword '{x}'"
        else
            preturn x

let KEYNAME: Parser<_> =
    IdentifierOptions(isAsciiIdStart = isAsciiLower) |> safeIdentifier

let IDENTIFIER: Parser<_> =
    IdentifierOptions(isAsciiIdStart = isAsciiUpper) |> safeIdentifier


/// Helper function, returns string with elements of x separated by commas
let private toCommaSepString x =
    x |> Seq.map (sprintf "%O") |> String.concat ", "

/// Helper parser: a "between" with optional whitespace after startc
let private enclose startc endc =
    between (ws (skipChar startc)) (skipChar endc)
...
let pstringEq str p = ws (skipString str) >>. ws EQ >>. p
...
let pquantifier =
    pipe3
        (ws <| choice [ stringReturn "forall" All; stringReturn "exists" Exists ])
        (ws IDENTIFIER)
        (ws KEYNAME)
