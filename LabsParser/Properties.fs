...
let propertyRef p =
    ...
    pipe3
        ...
        (choice [ followedBy OF >>. ws OF >>. pOf; preturn (None, None) ])
        (fun k offset (ofVar, ofNum) ->
            { Var = (k, ofVar)
              Offset = offset
              OfAgent = ofNum })
...
let pquantpred =
    let propertyLink = ws (skipString "id") >>. ws OF >>. ws KEYNAME

    let pbaseprop =
        makeBExprParser (makeExprParser propertyRef propertyLink (fail "ifelse in properties not supported yet"))

    pipe2 (sepEndBy pquantifier (ws COMMA) >>= toMap) pbaseprop (fun qs pred -> { Quantifiers = qs; Predicate = pred })
