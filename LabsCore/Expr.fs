...
let rec internal foldB foldExpr acc bexpr =
    ...
    match bexpr with
    | BLeaf _ -> acc
    | Compare(e1, _, e2) -> Seq.fold foldExpr acc [ e1; e2 ]
    | Neg b
    | ForEach(_, _, b) -> recurse acc b
...
let rec fold fleaf fref acc expr =
    ...
    | Leaf l -> fleaf acc l
    | Nondet(e1, e2, _)
    | Arithm(e1, _, e2) -> Seq.fold recurse acc [ e1; e2 ]
    | Unary(_, e) -> recurse acc e
    | RawCall(_, args) -> Seq.fold recurse acc args
    | IfElse(_, e1, e2) -> Seq.fold recurse acc [ e1; e2 ]
    | Ref r ->
        let newacc = fref acc r
        r.Offset |> Option.map (List.fold recurse newacc) |> Option.defaultValue newacc
...
let rec cata fleaf farithm funary fnondet fref fraw fif expr =
    ...
    | Ref r -> fref r.Var (Option.map (List.map recurse) r.Offset) (Option.map recurse r.OfAgent)
...
let getRefs expr =
    fold (fun a _ -> a) (fun a r -> Set.add r a) Set.empty expr
...
/// <returns>The value of <c>expr</c>.</returns>
let evalCexprNoId expr =
    evalConstExpr (fun _ -> failwith "id not allowed here.") expr
