...
let keys (source: Map<_, _>) =
    Seq.map (fun (KeyValue(k, _)) -> k) source

/// Returns the set of keys in source.
let values (source: Map<_, _>) =
    Seq.map (fun (KeyValue(_, v)) -> v) source
...
let union (source: Map<'Key, 'T>) (altSource: Map<'Key, 'T>) =
    unionWith (fun _ x _ -> x) source altSource
