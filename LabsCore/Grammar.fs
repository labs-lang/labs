...
let inline _name x =
    ...
    let setter { Pos = p; Def = d; Source = s } n' =
        { Name = n'
          Pos = p
          Def = d
          Source = s }
...
let inline _def x =
    ...
    let setter { Name = n; Pos = p; Source = s } d' =
        { Name = n
          Pos = p
          Def = d'
          Source = s }
...
let inline _pos x =
    ...
    let setter { Name = n; Def = d; Source = s } p' =
        { Name = n
          Pos = p'
          Def = d
          Source = s }
...
type Init =
    ...
    member this.Pick id_ =
        ...
        let pickRandom lst =
            List.item (rnd.Next(List.length lst)) lst
...
type Stmt<'a> =
    ...
    override this.ToString() =
        ...
        | Skip -> "√"
        | Act a -> string a
        | Block stmts -> List.map string stmts |> String.concat "; " |> sprintf "{ %s }"
        | Name s -> s
...
let inline isEnvVar v =
    match v.Location with
    | E -> true
    | _ -> false

let inline isLstigVar v =
    match v.Location with
    | L _ -> true
    | _ -> false
